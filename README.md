...
## TODO 4/17
'메타 데이터 추출' 용어를 '지도에 그리는 중' 으로 변경
지도 화면에서 사진을 클릭하면 사진 정보가 뜨는데, 좌우로 넘기면 시간순서대로 사진 정보가 이동되고, 지도도 이동되도록 기능 추가
지도 화면에서 사진을 클릭해서 문구를 입력 할 수 있게 기능 추가
사진에 추가된 문구는 동영상으로 제작시 사진 하단에 그래픽 폰트로 글자 표출 되도록 기능 추가
지도 화면에 포인트로 출력되는데, 시간대 별로 play 되도록 구현, 여행을 회상 할 수 있는 보조 기능
지도를 최대로 자세히 보기 했을 때, 최대 줌 단계가 필요함. 최대로 줌하면 화면이 사라지는데 그 전까지만 줌하도록 정리